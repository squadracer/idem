...
function log(level, o) {
	...
	document.querySelector("#popup-log").value += `\n${new Date().toISOString()} [${level}] - ${JSON.stringify(o)} (${new Error().stack.split('\n')[1].match(/[^\/]*$/)?.[0]})`
}
	...
	log_activate: function() {
		browser.runtime.sendMessage({
			...
			if (r?.res == 'ack') {
				...
				document.querySelector("#log_activate_action_id").textContent = 'Deactivate Logs'
				document.querySelector("#log_activate_action_id").setAttribute('action', 'log_deactivate')
			}
		})
	},
	...
	log_deactivate: function() {
		browser.runtime.sendMessage({
			...
			if (r?.res == 'ack') {
				...
				document.querySelector("#log_activate_action_id").textContent = 'Activate Logs'
				document.querySelector("#log_activate_action_id").setAttribute('action', 'log_activate')
			}
		})
	},
		...
		if (channel) {
			...
			browser.runtime.sendMessage({
				...
				if (r.res == 'ack') {
					...
					document.querySelector("#twitch_button").textContent = 'Deconnect from twitch'
					document.querySelector("#twitch_button").setAttribute('action', 'twitch_deconnect')
				}
			})
		} else {
	...
	twitch_deconnect: function() {
		browser.runtime.sendMessage({
			...
			if (r.res == 'ack') {
				...
				document.querySelector("#twitch_button").setAttribute('action', 'twitch_connect')
			}
		})
	},
