...
function blockLetterFunction(letter) {
	return e => {
		const code = e.detail.code.replaceAll(letter.toLowerCase(), '').replaceAll(letter.toUpperCase(), '')
...
const actions = {
	...
	clear: function() {
		document.removeEventListener('IDEToExternalEditor', clearIdeActionFunction)
		document.removeEventListener('IDEToExternalEditor', blockLetterActionFunction)
	...
	ide_block_letter: function(letter) {
		...
		if (letter) {
			...
			document.addEventListener('IDEToExternalEditor', blockLetterActionFunction)
	...
	ide_swap_submit: function() {
		...
		const submitImage = window.getComputedStyle(submit, ':before').backgroundImage
		...
		const playAllImage = window.getComputedStyle(playAll, ':before').backgroundImage
	...
	ide_deactivate_submit: function() {
		...
		const listener = e => {
			...
			} while (left_new <= e.clientX && e.clientX <= (left_new + submit.clientWidth | 0)
					&& top_new <= e.clientY && e.clientY <= (top_new + submit.clientHeight | 0));
